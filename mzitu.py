...
class mzitu():
    ...
    def mkdir(self, path): ##这个函数创建文件夹
        path = path.strip()
        isExists = os.path.exists(os.path.join("/Users/Edward/Pictures/mzitu", path))
        if not isExists:
            print(u'建了一个名字叫做', path, u'的文件夹！')
            os.makedirs(os.path.join("/Users/Edward/Pictures/mzitu", path))
            os.chdir(os.path.join("/Users/Edward/Pictures/mzitu", path)) ##切换到目录
            return True
        else:
